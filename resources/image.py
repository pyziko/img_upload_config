...
class ImageUpload(Resource):
    ...
    def post(cls):
        ...
        folder = f"user_{user_id}"  # static/images/user_1  # root folder configured in default_config
        try:
...
class Image(Resource):
    ...
    def get(cls, filename: str):
        """
        Returns the requested image if it exists. Looks up the logged in user's folder
    ...
    def delete(cls, filename: str):
        ...
        try:
            ...
            return {"message": getText("images_deleted").format(filename)}, 200
        except FileNotFoundError:
            return {"message": getText("image_not_found").format(filename)}, 404
        except:
            traceback.print_exc()
...
class Avatar(Resource):
    # we can access it via {{url}}/static/images/avatars/user_1.jpg
    @classmethod
